...
Map<String, String> _clientFiles(
  ...
  return {
    ...
    'params.dart': _paramsLibrary(routes),
    'inputs.dart': _inputsLibrary(typedData.inputFiles.values),
    'models.dart': _modelsLibrary(models.filesByName.values),
    for (final node in _routeNodes(routes))
      node.filePath: _routeEntry(node, typedData.routeDataTypes),
    for (final node in _paramNodes(routes))
      node.paramsFilePath: _paramsEntry(node),
    for (final input in typedData.inputFiles.values) input.filePath: input.source,
    for (final model in models.filesByName.values) model.filePath: model.source,
  };
}
...
_ClientTypedDataResult? _buildClientTypedDataForRoute(
  ...
  if (inputType case _ClientInputObjectType(isModel: true, filePath: final filePath?)) {
    return _ClientTypedDataResult(
      reference: _ClientTypedDataRef(typeName: inputType.className, filePath: filePath),
    );
  }
