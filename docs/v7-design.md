...
This aligns directly with roux 0.3's native duplicate-policy and multi-match behavior, so Spry does not need a separate scope-matching adapter layer for middleware or errors.
...
`Locals` is a zero-cost `extension type` over `Map<Symbol, Object?>`.

```dart
extension type Locals(Map<Symbol, Object?> _values)
    implements Map<Symbol, Object?> {
  T? get<T>(Symbol key) => _values[key] as T?;

  void set(Symbol key, Object? value) {
    _values[key] = value;
  }

  bool has(Symbol key) => _values.containsKey(key);
}
```
...
Spry relies directly on roux 0.3 for:

- `match(...)` for handler lookup
- `matchAll(...)` for middleware and error scope collection
- built-in `DuplicatePolicy` handling at registration time
...
- The v7 skeleton currently keeps these runtime entry types under `lib/src/`
- Top-level export shims such as `lib/spry.dart` and `lib/app.dart` have not been restored yet
- Reintroducing those shims later should not change the runtime model described here
...
The current v7 skeleton keeps the runtime surface directly under `lib/src/`:
...
When those shims return, they should map to the current files roughly like this:
...
export 'src/builder/config.dart';
export 'src/builder/route_tree.dart';
export 'src/builder/scanner.dart';
...
- **Locals** — zero-cost `extension type` over `Map<Symbol, Object?>` with `get`, `set`, `has`.
...
- **Registration semantics** — duplicate behavior is resolved during registration through roux `DuplicatePolicy`; matching only sees the effective registered entries.
...
- **Current package layout** — v7 runtime types currently live directly under `lib/src/`; top-level export shims will be restored later after the API shape settles.
