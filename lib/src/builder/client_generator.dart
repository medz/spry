...
Stream<GeneratedEntry> generateClientEntries(
  ...
  yield _clientSourceEntry(
    ...
    outputDir,
    'params.dart',
    _modelTypedLibrary(typedParams.files.values),
  );
  yield _clientSourceEntry(
    config.rootDir,
    outputDir,
    'inputs.dart',
    _modelTypedLibrary(typedData.files.values),
  );
  yield _clientSourceEntry(
    config.rootDir,
    outputDir,
  ...
  yield _clientSourceEntry(
    ...
    outputDir,
    'models.dart',
    _modelTypedLibrary(models.filesByName.values),
  );
...
String _modelTypedLibrary(Iterable<_ClientModelTypedFile> files) {
  final exports = [
    for (final file in [
      ...files,
...
_ClientModelTypedBuildResult _buildClientTypedParams(_ClientRootRoutes routes) {
  final files = <String, _ClientModelTypedFile>{};
  for (final node in _paramNodes(routes)) {
    final file = _ClientModelTypedFile(
  ...
  return _ClientModelTypedBuildResult(files: files);
}
...
_ClientModelTypedBuildResult _buildClientTypedData(
  _ClientRootRoutes routes,
  String routesRootDir,
  _ClientModelRegistry models,
) {
  final routeDataTypes = <RouteEntry, _ClientModelTypedRef>{};
  final files = <String, _ClientModelTypedFile>{};
  for (final node in _routeNodes(routes)) {
    for (final route in node.routes) {
  ...
  return _ClientModelTypedBuildResult(
...
_ClientModelTypedResult? _buildClientTypedDataForRoute(
  RouteEntry route,
  ...
  if (inputType case _ClientInputObjectType(
    ...
    return _ClientModelTypedResult(
      reference: _ClientModelTypedRef(
  ...
  final inputFile = _ClientModelTypedFile(
    filePath:
...
String _routeEntry(
  _ClientRouteNode node,
  Map<RouteEntry, _ClientModelTypedRef> routeDataTypes,
...
final class _ClientModelTypedRef {
  const _ClientModelTypedRef({required this.filePath, required this.typeName});

  final String filePath;
  final String typeName;
}

final class _ClientModelTypedResult {
  const _ClientModelTypedResult({required this.reference, this.file});

  final _ClientModelTypedRef reference;
  final _ClientModelTypedFile? file;
}

final class _ClientModelTypedBuildResult {
  const _ClientModelTypedBuildResult({
    this.routeDataTypes = const {},
    required this.files,
  });

  final Map<RouteEntry, _ClientModelTypedRef> routeDataTypes;
  final Map<String, _ClientModelTypedFile> files;
}
...
final class _ClientModelTypedFile extends _ClientModelTypedRef {
  const _ClientModelTypedFile({
...
final class _ClientModelRegistry {
  final Map<String, _ClientModelTypedFile> filesByName = {};
  ...
  _ClientInputObjectType? resolveSchema(
    ...
    try {
      ...
      filesByName[name] = _ClientModelTypedFile(
