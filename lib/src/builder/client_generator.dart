...
Stream<GeneratedEntry> generateClientEntries(
  ...
  final typedHeaders = _buildClientTypedHeaders(routes, routesRootDir, models);
  final typedQueries = _buildClientTypedQueries(routes, routesRootDir, models);
  ...
  yield _clientSourceEntry(
    ...
    outputDir,
    'headers.dart',
    _headersLibrary(typedHeaders.headerFiles.values),
  );
  yield _clientSourceEntry(
    config.rootDir,
    outputDir,
    'queries.dart',
    _queriesLibrary(typedQueries.queryFiles.values),
  );
  yield _clientSourceEntry(
    config.rootDir,
    outputDir,
  ...
  for (final node in _routeNodes(routes)) {
    yield _clientSourceEntry(
      ...
      _routeEntry(
        ...
        typedHeaders.routeHeaderTypes,
        typedQueries.routeQueryTypes,
  ...
  for (final header in typedHeaders.headerFiles.values) {
    yield _clientSourceEntry(
      config.rootDir,
      outputDir,
      header.filePath,
  ...
  for (final query in typedQueries.queryFiles.values) {
    yield _clientSourceEntry(
      config.rootDir,
      outputDir,
      query.filePath,
...
String _queriesLibrary(Iterable<_ClientQueryFile> queries) {
  final exports = [
    for (final query in [
      ...queries,
    ]..sort((a, b) => a.filePath.compareTo(b.filePath)))
      "export '${query.filePath}';",
  ].join('\n');
  return '''// Generated by `spry build client`.
// ignore_for_file: public_member_api_docs, file_names
...
String _headersLibrary(Iterable<_ClientHeaderFile> headers) {
  final exports = [
    for (final header in [
      ...headers,
    ]..sort((a, b) => a.filePath.compareTo(b.filePath)))
      "export '${header.filePath}';",
  ].join('\n');
  return '''// Generated by `spry build client`.
// ignore_for_file: public_member_api_docs, file_names
...
_ClientTypedHeaderBuildResult _buildClientTypedHeaders(
  _ClientRootRoutes routes,
  String routesRootDir,
  _ClientModelRegistry models,
) {
  final routeHeaderTypes = <RouteEntry, _ClientTypedHeaderRef>{};
  final headerFiles = <String, _ClientHeaderFile>{};
  for (final node in _routeNodes(routes)) {
    for (final route in node.routes) {
      final headerType = _buildClientTypedHeaderForRoute(
        route,
        node,
        routesRootDir,
        models,
      ...
      routeHeaderTypes[route] = headerType.reference;
      headerFiles[headerType.headerFile.filePath] = headerType.headerFile;
    }
  }
  return _ClientTypedHeaderBuildResult(
    routeHeaderTypes: routeHeaderTypes,
    headerFiles: headerFiles,
  );
}

_ClientTypedQueryBuildResult _buildClientTypedQueries(
  _ClientRootRoutes routes,
  String routesRootDir,
  _ClientModelRegistry models,
) {
  final routeQueryTypes = <RouteEntry, _ClientTypedQueryRef>{};
  final queryFiles = <String, _ClientQueryFile>{};
  for (final node in _routeNodes(routes)) {
    for (final route in node.routes) {
      final queryType = _buildClientTypedQueryForRoute(
        route,
        node,
        routesRootDir,
        models,
      ...
      routeQueryTypes[route] = queryType.reference;
      queryFiles[queryType.queryFile.filePath] = queryType.queryFile;
    }
  }
  return _ClientTypedQueryBuildResult(
    routeQueryTypes: routeQueryTypes,
...
_ClientTypedHeaderResult? _buildClientTypedHeaderForRoute(
  RouteEntry route,
  _ClientRouteNode node,
  String routesRootDir,
  _ClientModelRegistry models,
) {
  final parameters = _typedHeaderParameters(route, models);
  if (parameters.isEmpty) {
    return null;
  }

  final className = _headerClassName(route, node);
  final headerFile = _ClientHeaderFile(
    filePath:
        'headers/${_sourceOperationFileSegments(route, routesRootDir).join('/')}.dart',
    typeName: className,
    source: _headerEntry(parameters, className),
    required: parameters.any((parameter) => parameter.required),
  );
  return _ClientTypedHeaderResult(
    reference: _ClientTypedHeaderRef(
      filePath: headerFile.filePath,
      typeName: headerFile.typeName,
      required: headerFile.required,
    ),
    headerFile: headerFile,
  );
}

_ClientTypedQueryResult? _buildClientTypedQueryForRoute(
  RouteEntry route,
  _ClientRouteNode node,
  String routesRootDir,
  _ClientModelRegistry models,
) {
  final parameters = _typedQueryParameters(route, models);
  if (parameters.isEmpty) {
    return null;
  }

  final className = _queryClassName(route, node);
  final queryFile = _ClientQueryFile(
    filePath:
        'queries/${_sourceOperationFileSegments(route, routesRootDir).join('/')}.dart',
    typeName: className,
    source: _queryEntry(parameters, className),
    required: parameters.any((parameter) => parameter.required),
  );
  return _ClientTypedQueryResult(
    reference: _ClientTypedQueryRef(
      filePath: queryFile.filePath,
      typeName: queryFile.typeName,
      required: queryFile.required,
    ),
    queryFile: queryFile,
  );
}
...
String _routeEntry(
  ...
  Map<RouteEntry, _ClientTypedHeaderRef> routeHeaderTypes,
  Map<RouteEntry, _ClientTypedQueryRef> routeQueryTypes,
...
List<_ClientQueryField> _typedQueryParameters(
  RouteEntry route,
  _ClientModelRegistry models,
) {
  final openapi = route.openapi;
  ...
  final fields = <_ClientQueryField>[];
  for (final rawParameter in rawParameters) {
    final parameter = _resolveParameterObject(
      rawParameter,
      components: components,
    );
    if (parameter is! Map<String, Object?> || parameter['in'] != 'query') {
      continue;
    }
...
List<_ClientHeaderField> _typedHeaderParameters(
  RouteEntry route,
  _ClientModelRegistry models,
) {
  final openapi = route.openapi;
  ...
  final components = _routeComponents(route);
  final usedNames = <String>{};
  final fields = <_ClientHeaderField>[];
  for (final rawParameter in rawParameters) {
    final parameter = _resolveParameterObject(
      rawParameter,
      components: components,
    );
    if (parameter is! Map<String, Object?> || parameter['in'] != 'header') {
      continue;
    }
    final name = parameter['name'];
    final schema = parameter['schema'];
...
String _headerClassName(RouteEntry route, _ClientRouteNode node) {
  return switch (route.method) {
    null => '${node.classStem.join()}Headers',
    final method => '${_pascal(method.name)}${node.classStem.join()}Headers',
  };
}

String _queryClassName(RouteEntry route, _ClientRouteNode node) {
  return switch (route.method) {
    null => '${node.classStem.join()}Query',
    final method => '${_pascal(method.name)}${node.classStem.join()}Query',
  };
}
...
String _queryEntry(List<_ClientQueryField> fields, String className) {
  final constructorParameters = fields
      ...
      .join(', ');
  final mapEntries = fields.map(_queryFieldMapEntry).join('\n');
  return '''// Generated by `spry build client`.
// ignore_for_file: public_member_api_docs, file_names

import 'package:spry/client.dart';

extension type $className._(URLSearchParams _) implements URLSearchParams {
  factory $className({$constructorParameters}) {
    return $className.raw({
${mapEntries.isEmpty ? '' : '$mapEntries\n'}    });
  }

  factory $className.raw([Object? init]) => $className._(URLSearchParams(init));
}
''';
}

String _headerEntry(List<_ClientHeaderField> fields, String className) {
  final constructorParameters = fields
      .map(
        (field) => field.required
            ? 'required ${field.type.typeAnnotation()} ${field.name}'
      ...
      .join(', ');
  final mapEntries = fields.map(_headerFieldMapEntry).join('\n');
  return '''// Generated by `spry build client`.
// ignore_for_file: public_member_api_docs, file_names

import 'package:spry/client.dart';

extension type $className._(Headers _) implements Headers {
  factory $className({$constructorParameters}) {
    return $className.raw({
${mapEntries.isEmpty ? '' : '$mapEntries\n'}    });
  }

  factory $className.raw([Object? init]) => $className._(Headers(init));
}
''';
}

String _queryFieldMapEntry(_ClientQueryField field) {
  final key = _dartString(field.queryName);
...
String _headerFieldMapEntry(_ClientHeaderField field) {
  final key = _dartString(field.headerName);
  final value = _queryScalarEncodeExpression(
    field.type,
    field.name,
    optional: !field.required,
...
final class _ClientTypedHeaderRef {
  const _ClientTypedHeaderRef({
...
final class _ClientTypedHeaderResult {
  const _ClientTypedHeaderResult({
    required this.reference,
    required this.headerFile,
  });

  final _ClientTypedHeaderRef reference;
  final _ClientHeaderFile headerFile;
}

final class _ClientTypedHeaderBuildResult {
  const _ClientTypedHeaderBuildResult({
    required this.routeHeaderTypes,
    required this.headerFiles,
  });

  final Map<RouteEntry, _ClientTypedHeaderRef> routeHeaderTypes;
  final Map<String, _ClientHeaderFile> headerFiles;
}

final class _ClientTypedQueryRef {
  const _ClientTypedQueryRef({
    required this.filePath,
    required this.typeName,
    required this.required,
  });
...
final class _ClientTypedQueryResult {
  const _ClientTypedQueryResult({
    required this.reference,
    required this.queryFile,
  });

  final _ClientTypedQueryRef reference;
  final _ClientQueryFile queryFile;
}

final class _ClientTypedQueryBuildResult {
  const _ClientTypedQueryBuildResult({
    required this.routeQueryTypes,
    required this.queryFiles,
  });

  final Map<RouteEntry, _ClientTypedQueryRef> routeQueryTypes;
  final Map<String, _ClientQueryFile> queryFiles;
}
...
final class _ClientHeaderFile extends _ClientTypedHeaderRef {
  const _ClientHeaderFile({
    required super.filePath,
    required super.typeName,
...
final class _ClientQueryFile extends _ClientTypedQueryRef {
  const _ClientQueryFile({
    required super.filePath,
    required super.typeName,
    required super.required,
    required this.source,
...
final class _ClientQueryField {
  const _ClientQueryField({
    required this.name,
...
final class _ClientHeaderField {
  const _ClientHeaderField({
    required this.name,
